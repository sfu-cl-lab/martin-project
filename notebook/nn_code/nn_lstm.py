...
class vball_lstm:
    def __init__(self, FEATURE_COUNT, LSTM_CELL_COUNT, MAX_TRACE_LENGTH, learning_rate, rnn_type='bp_last_step'):
        """
        define a dynamic LSTM
        """
        with tf.name_scope("LSTM_layer"):
            self.rnn_input = tf.placeholder(tf.float32, [None, 10, FEATURE_COUNT], name="rnn_input") #x_1
            self.trace_lengths = tf.placeholder(tf.int32, [None], name="trace_lengths") #tl  

            self.lstm_cell = tf.contrib.rnn.LSTMCell(num_units=LSTM_CELL_COUNT, state_is_tuple=True, initializer=tf.random_uniform_initializer(-0.05, 0.05))
            ...
            # Indexing
            self.rnn_last = tf.gather(tf.reshape(self.outputs, [-1, LSTM_CELL_COUNT]), self.index)
        ...
        with tf.name_scope("Dense_Layer_first"):
            self.W1 = tf.get_variable('w1_xaiver', [num_layer_1, num_layer_2], initializer=tf.contrib.layers.xavier_initializer())
